    ...
    @endphp
      <tr data-id="{{ $list->id }}">
        ...
        <td class="d-flex align-items-center">
            ...
            <input type="number" class="form-control qty mx-1 text-center" readonly value="{{ $list->qty }}" data-id="{{ $list->id }}" min="1" max="10" style="width: 60px;">
            <button type="button" class="btn btn-sm btn-outline-secondary btn-plus">+</button>
          </td>
        <td class="item-total">{{ $list->price * $list->qty }}</td>
        <td>
            <a href="{{url('/delete-cart-item')}}/{{$list->id}}"><button class="btn btn-sm btn-danger btn-delete">×</button></a>
          </td>
      </tr>
      @endforeach
      @else
